...
// Запис/читання блокувань — через службовий ключ (маршрут захищений входом адміна),
// щоб збереження гарантовано проходило незалежно від стану сесії/RLS.
...
// POST /api/blocked — { action: 'set', spots: number[] } | 'block' | 'unblock' | 'clear'
export async function POST(req: Request) {
  ...
  // Зберегти ВЕСЬ набір заблокованих місць одним запитом (кнопка «Зберегти»).
  if (body.action === 'set') {
