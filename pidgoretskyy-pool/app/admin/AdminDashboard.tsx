...
type Tab = 'dashboard' | 'bookings' | 'calendar' | 'manual' | 'blocked' | 'closures' | 'logins';
...
export default function AdminDashboard({ email }: { email: string }) {
  ...
  const [section, setSection] = useState<'pool' | 'hotel'>('pool');
  ...
  const reload = useCallback(async () => {
    try {
      const [b, bl, lg, rb, st] = await Promise.all([
  ...
  const statCards = [
    ...
    { label: 'Зайнятість', value: `${occupied.size}/40`, sub: 'шезлонгів сьогодні', subColor: '#86868B' },
  ];
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.05 }}>
            ...
            <span style={{ fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: '#86868B', marginTop: 2 }}>
              Адмін-панель · {section === 'hotel' ? 'Готель' : 'Басейн'}
            </span>
          </div>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '24px 22px 70px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['pool', 'hotel'] as const).map((s) => (
            ...
            >
              {s === 'pool' ? 'Басейн' : 'Готель'}
            </button>
          ))}
        </div>

        {/* ГОЛОВНИЙ ВИМИКАЧ: пауза бронювання на всьому сайті */}
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
        ...
        {loading ? (
          ...
          <HotelSection roomBookings={roomBookings} onToast={show} />
        ) : (
          <>
            {/* ─── Огляд ─────────────────────────────────────────────── */}
            {tab === 'dashboard' && (
...
function HotelBookings({ roomBookings }: { roomBookings: RoomBooking[] }) {
...
function HotelSection({ roomBookings, onToast }: { roomBookings: RoomBooking[]; onToast: (m: string) => void }) {
  const [hotelTab, setHotelTab] = useState<'overview' | 'bookings' | 'calendar' | 'closures'>('overview');
  ...
  const kpis = [
    ...
    { label: 'Дохід', value: `${revenue} ₴`, sub: 'оплачено' },
  ];
  ...
  const tabBtn = (k: 'overview' | 'bookings' | 'calendar' | 'closures', label: string) => (
    <button
      key={k}
  ...
  return (
    <div style={{ animation: 'fadeUp .4s ease' }}>
      ...
      {hotelTab === 'bookings' && <HotelBookings roomBookings={roomBookings} />}
                  ...
                  <div key={room} style={{ display: 'grid', gridTemplateColumns: `140px repeat(${CAL_DAYS}, 1fr)`, borderBottom: '1px solid #F5F5F7' }}>
                    ...
                    {days.map((d) => {
                      ...
                      return (
                        <div
                          key={d}
                          title={b ? `${b.code} · ${b.name} (${fmtDateHB(b.check_in)}→${fmtDateHB(b.check_out)})` : ''}
                          style={{
