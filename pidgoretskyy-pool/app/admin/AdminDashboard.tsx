...
export default function AdminDashboard({ email }: { email: string }) {
  ...
  const reload = useCallback(async () => {
    try {
      const [b, bl, lg, rb] = await Promise.all([
    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF', color: '#1D1D1F' }}>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '24px 22px 70px' }}>
        ...
        {loading ? (
          ...
          <HotelSection roomBookings={roomBookings} />
        ) : (
          <>
            {/* ─── Огляд ─────────────────────────────────────────────── */}
            {tab === 'dashboard' && (
...
function HotelSection({ roomBookings }: { roomBookings: RoomBooking[] }) {
  const [hotelTab, setHotelTab] = useState<'overview' | 'bookings' | 'calendar'>('overview');
  ...
  const tabBtn = (k: 'overview' | 'bookings' | 'calendar', label: string) => (
    <button
      key={k}
