...
export default function AdminDashboard({ email }: { email: string }) {
  ...
  // ── Блокування місць (миттєве, з підтвердженням і перевіркою) ─────────
  const toggleBlock = async (spot: number) => {
    if (blockBusy) return;
    const wasBlocked = blocked.includes(spot);
    // оптимістично оновлюємо, щоб клік був миттєвим
    setBlocked((prev) => (wasBlocked ? prev.filter((x) => x !== spot) : [...prev, spot].sort((a, b) => a - b)));
    setBlockBusy(true);
    try {
      const r = await fetch('/api/blocked', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ spot, action: wasBlocked ? 'unblock' : 'block' }),
      });
      if (!r.ok) {
        let msg = 'Не вдалося зберегти. Спробуйте ще раз.';
        try {
          const j = await r.json();
          if (j?.error) msg = `Помилка: ${j.error}`;
        } catch {
          /* ignore */
        }
        // відкат
        setBlocked((prev) => (wasBlocked ? [...prev, spot].sort((a, b) => a - b) : prev.filter((x) => x !== spot)));
        show(msg);
      } else {
