...
/** Зміщення київського часу відносно UTC (мс) для конкретної дати — з урахуванням літнього часу. */
function kyivOffsetMs(dateStr: string): number {
  const probe = new Date(`${dateStr}T12:00:00Z`);
  const asKyiv = new Date(probe.toLocaleString('en-US', { timeZone: 'Europe/Kyiv' }));
  const asUtc = new Date(probe.toLocaleString('en-US', { timeZone: 'UTC' }));
  return asKyiv.getTime() - asUtc.getTime();
}

/**
 * ISO-мітка часу для конкретної години ЗА КИЇВСЬКИМ часом.
 * Напр. kyivInstant('2026-07-26', 23, 59, 59) — кінець дня 26 липня у Львові.
 */
export function kyivInstant(dateStr: string, hh = 23, mm = 59, ss = 59): string {
  const wall = Date.UTC(
  ...
  return new Date(wall - kyivOffsetMs(dateStr)).toISOString();
}
