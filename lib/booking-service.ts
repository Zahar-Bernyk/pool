...
export async function createBooking(
  ...
  // Ручні броні позначаємо окремо: неоплачена ручна бронь має лишатись у списку,
  // тоді як покинутий резерв із сайту автоматично скасовується через 15 хв.
  if ((input as { manual?: unknown }).manual === true) {
    await supabase.from('bookings').update({ manual: true }).eq('id', booking.id);
    (booking as Booking & { manual?: boolean }).manual = true;
  }
