...
async function takenRooms(checkIn: string, checkOut: string): Promise<Set<number>> {
  ...
  const [{ data }, { data: closures }] = await Promise.all([
    ...
    // Закриття, що перетинаються з [checkIn, checkOut)
    supabase
      .from('room_closures')
      .select('room, start_date, end_date, weekdays')
  ...
  // Дні проживання [checkIn, checkOut) як набір днів тижня (0=нд … 6=сб).
  const stayDows = new Set<number>();
  {
    ...
    while (cur < end) {
      stayDows.add(new Date(cur).getUTCDay());
      cur += 86400000;
    }
  }

  type Closure = { room: number | null; weekdays: number[] | null };
  (closures || []).forEach((r: Closure) => {
    // Якщо задано дні тижня — закриття діє лише якщо хоч один день
    // проживання припадає на ці дні.
    if (r.weekdays && r.weekdays.length > 0) {
      const hit = r.weekdays.some((w) => stayDows.has(w));
      if (!hit) return;
    }
    if (r.room === null) ALL_ROOMS.forEach((n) => s.add(n));
    else s.add(r.room);
  });
...
export async function createRoomBooking(
  ...
  // Якщо вказано конкретний номер (ручне бронювання з календаря) — беремо лише його.
  const wanted = Number((input as { room?: number }).room);
  const candidates =
    Number.isInteger(wanted) && wanted > 0
      ? cat.rooms.filter((r) => r === wanted && !taken.has(r))
      : cat.rooms.filter((r) => !taken.has(r));
  ...
  for (const room of candidates) {
    ...
    if (!error) {
      ...
      // Пошту зберігаємо окремо: RPC її не приймає, а міняти сигнатуру функції
      // ризиковано. Якщо запис не вдасться — бронювання все одно чинне.
      const email = String((input as { email?: unknown }).email || '').trim().toLowerCase();
      const isManual = (input as { manual?: unknown }).manual === true;
      const extra: Record<string, unknown> = {};
      if (email) extra.email = email;
      if (isManual) extra.manual = true;
      if (Object.keys(extra).length > 0) {
        await supabase.from('room_bookings').update(extra).eq('id', booking.id);
        Object.assign(booking, extra);
      }
      return { ok: true, booking };
    }

    // Кімнату щойно зайняли або її закрито — пробуємо наступну.
    const m = error.message || '';
    if (m.includes('BOOKINGS_CLOSED')) {
      return { ok: false, error: 'CLOSED', message: 'Бронювання на сайті тимчасово призупинено' };
    }
    if (m.includes('ROOM_TAKEN') || m.includes('ROOM_CLOSED')) continue;
