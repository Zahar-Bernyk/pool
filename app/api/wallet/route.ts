...
import { fmtDate, kyivInstant } from '@/lib/dates';
...
export async function GET(req: Request) {
  if (!allow(`wallet:${clientIp(req)}`, 30, 60_000)) return tooMany();
  ...
  // Квиток містить імʼя гостя, тож самого коду недостатньо: потрібен ще
  // невгадуваний ідентифікатор бронювання (?t=...). Він є в посиланні,
  // куди LiqPay повертає гостя, і в адмін-панелі.
  if (!UUID_RE.test(token)) {
    return NextResponse.json(
      { error: 'TOKEN_REQUIRED', message: 'Відкрийте квиток за посиланням після оплати або з адмін-панелі.' },
      { status: 403 },
    );
  }
  ...
  try {
    if (code.startsWith('PM-')) {
      ...
      const { data } = await supabase
        .from('bookings')
        .select('id, code, name, date, session, spots, adults, children, amount, paid, status')
        .eq('code', code)
        .maybeSingle();

      if (!data || data.id !== token) return NextResponse.json({ error: 'Not found' }, { status: 404 });
      ...
      const buf = await buildPkpass({
        ...
        // Актуальний із 10:00 і недійсний після кінця того самого дня (за київським часом)
        relevantDate: kyivInstant(data.date, 10, 0, 0),
        expirationDate: kyivInstant(data.date, 23, 59, 59),
      });
    ...
    const { data } = await supabase
      .from('room_bookings')
      .select('id, code, name, room, category, check_in, check_out, nights, guests, extra_bed, amount, paid, status')
      .eq('code', code)
      .maybeSingle();
    ...
    const buf = await buildPkpass({
      ...
      // Актуальний із 14:00 у день заїзду, недійсний після 12:00 у день виїзду
      relevantDate: kyivInstant(data.check_in, 14, 0, 0),
      expirationDate: kyivInstant(data.check_out, 12, 0, 0),
    });
