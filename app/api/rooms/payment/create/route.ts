...
export async function POST(req: Request) {
  ...
  // Телефон обовʼязковий і має бути справжнім українським мобільним.
  const normPhone = normalizePhone(body?.phone);
  if (!normPhone) {
    return NextResponse.json({ ok: false, error: 'BAD_PHONE', message: PHONE_HINT }, { status: 400 });
  }
  body.phone = normPhone;

  // Пошта обовʼязкова: на неї надсилаємо підтвердження й повідомлення про зміни.
  const email = String((body as { email?: unknown }).email || '').trim().toLowerCase();
  if (!isValidEmail(email)) {
    return NextResponse.json(
      { ok: false, error: 'BAD_EMAIL', message: 'Вкажіть коректну електронну пошту.' },
      { status: 400 },
    );
  }
  (body as { email?: string }).email = email;
  ...
  const checkout = buildLiqpayCheckout({
    ...
    resultUrl: `${site}/hotel?paid=${b.code}&t=${b.id}`,
