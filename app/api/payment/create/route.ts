...
export async function POST(req: Request) {
  ...
  // Телефон обовʼязковий для онлайн-бронювання і має бути справжнім українським.
  // (Ручні бронювання з адмінки йдуть іншим маршрутом і цієї перевірки не мають.)
  const normPhone = normalizePhone(body?.phone);
  if (!normPhone) {
    return NextResponse.json(
      { ok: false, error: 'BAD_PHONE', message: PHONE_HINT },
      { status: 400 },
    );
  }
  body.phone = normPhone; // зберігаємо в єдиному форматі +380XXXXXXXXX
  ...
  const checkout = buildLiqpayCheckout({
    ...
    resultUrl: `${site}/booking?paid=${result.booking.code}&t=${result.booking.id}`,
