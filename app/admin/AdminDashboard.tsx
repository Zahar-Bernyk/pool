    ...
    try {
      for (const d of days) {
        const r = await fetch('/api/closures', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ start_date: d, end_date: d, note: 'Закрито з календаря' }),
        });
...
function HotelSection({
  ...
  const [hForm, setHForm] = useState<{ open: boolean; name: string; phone: string; nights: number; guests: number; extraBed: boolean }>(
    { open: false, name: '', phone: '', nights: 1, guests: 2, extraBed: false },
  );
    ...
    try {
      for (const c of cells) {
        const r = await fetch('/api/rooms/closures', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ room: c.room, start_date: c.date, end_date: c.date, note: 'Закрито з календаря' }),
        });
        if (!r.ok) throw new Error();
      }
      ...
      {hCell && (() => {
        ...
        const submit = async () => {
          ...
          try {
            ...
            setHCell(null);
            setHForm({ open: false, name: '', phone: '', nights: 1, guests: 2, extraBed: false });
            onChanged();
          } catch {
            onToast('Помилка мережі.');
          } finally {
