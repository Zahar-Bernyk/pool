...
import { fmtDate, todayStr, minBookingDate } from '@/lib/dates';
...
function BookingInner() {
  ...
  const [date, setDate] = useState<string>(minBookingDate());
  ...
  const loadAvailability = useCallback(async (d: string, s: Session) => {
    ...
    try {
      ...
      // Не змогли дізнатись зайнятість — НЕ показуємо місця вільними.
      setAvailReady(false);
      setAvailError(true);
    }
  }, []);
  ...
  // Оновити доступність при вході на крок вибору місць
  // (раптом адміністратор щойно щось заблокував).
  useEffect(() => {
    if (step === 'map') loadAvailability(date, session);
  ...
  useEffect(() => {
    ...
    if (tok) setPayToken(tok);
    if (!code) return;
    let stop = false;
    const poll = async (attempt = 0) => {
      try {
        const r = await fetch(`/api/payment/status?code=${code}${tok ? `&t=${tok}` : ''}`, { cache: 'no-store' });
        if (r.ok) {
          const j = await r.json();
  ...
  async function savePdf() {
    ...
    try {
      await downloadTicketPdf({
        subtitle: 'Басейн · Електронний квиток',
        code: confirmed.code,
        rows: [
          ...(confirmed.name ? [{ label: 'Гість', value: confirmed.name }] : []),
          ...confRows.map((r) => ({ label: r.k, value: r.v })),
        ],
        amount: confTotal,
        note: 'Покажіть цей квиток на вході. Гарного відпочинку!',
      });
    } catch {
          ...
          >
            <div>
              ...
              {availError && (
                ...
                >
                  <span>Не вдалося завантажити зайнятість місць. Оберіть заново, щоб не забронювати зайняте.</span>
                  <button
                    onClick={() => loadAvailability(date, session)}
                    style={{
                      flexShrink: 0,
              ...
              {!availReady && !availError && (
                <div style={{ color: '#86868B', fontSize: 14, marginBottom: 18 }}>
                  Завантажуємо зайнятість місць…
                </div>
              )}
            ...
            <div style={{ position: 'sticky', top: 90 }} className="map-sidebar">
              <div style={card}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 14 }}>
                  ...
                  <Legend color="repeating-linear-gradient(45deg,#E4B4B4,#E4B4B4 4px,#CE9494 4px,#CE9494 8px)" border="#C58585" label="Зайнято" />
                  <Legend color="repeating-linear-gradient(45deg,#BFBFC6,#BFBFC6 4px,#AEAEB5 4px,#AEAEB5 8px)" label="Заблоковано" />
                </div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (availError || !availReady) {
                        show('Зачекайте: завантажуємо зайнятість місць');
                        loadAvailability(date, session);
                        return;
                      }
                    ...
                    disabled={closed || !availReady}
                    style={{ ...primaryBtnFull, opacity: (closed || !availReady) ? 0.5 : 1, cursor: (closed || !availReady) ? 'not-allowed' : 'pointer' }}
                  >
                    Далі →
                  </button>
                </div>
            ...
            <div id="ticket" style={{ ...card, textAlign: 'left', padding: 26 }}>
              ...
              {confirmed && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 22 }}>
                  ...
                  <div style={{ fontSize: 12, color: '#86868B', marginTop: 8 }}>Покажіть цей код на вході</div>
                </div>
              )}
            </div>

            <div className="ticket-hide-print" style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
              <button onClick={savePdf} style={{ ...ghostBtn, flex: '1 1 160px' }}>
                Завантажити PDF
              </button>
              <button onClick={() => window.print()} style={{ ...ghostBtn, flex: '1 1 120px' }}>
                Друк
              </button>
              {isApple && confirmed && payToken && (
                <a
                  href={`/api/wallet?code=${confirmed.code}&t=${payToken}`}
                  style={{
                    flex: '1 1 100%',
                    display: 'flex',
                    alignItems: 'center',
              ...
              <button onClick={resetClient} style={{ ...primaryBtnFull, flex: '1 1 100%' }}>
                Нове бронювання
              </button>
            </div>
          </div>
