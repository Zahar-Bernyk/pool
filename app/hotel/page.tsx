...
/**
 * Чи випадає хоча б одна ніч проживання на пʼятницю або суботу.
 * Саме в ці вечори у комплексі найчастіше проходять весілля.
 */
function hasWeekendNight(checkIn: string, checkOut: string): boolean {
  if (!checkIn || !checkOut || checkOut <= checkIn) return false;
  const d = new Date(`${checkIn}T00:00:00Z`);
  const end = new Date(`${checkOut}T00:00:00Z`);
  while (d < end) {
    const dow = d.getUTCDay(); // 5 = пʼятниця, 6 = субота
...
function HotelInner() {
  ...
  const [noiseAck, setNoiseAck] = useState(false);
  ...
  useEffect(() => {
    ...
    const poll = async (attempt = 0) => {
      try {
        const r = await fetch(`/api/rooms/payment/status?code=${code}${tok ? `&t=${tok}` : ''}`, { cache: 'no-store' });
        if (r.ok) {
          const j = await r.json();
  ...
  const doPay = async () => {
    ...
    if (!isValidEmail(email)) {
      show('Вкажіть електронну пошту — на неї надійде підтвердження бронювання.');
      return;
    }
    setPaying(true);
    try {
        ...
        {step === 'details' && (
          <div style={{ animation: 'fadeUp .4s ease', maxWidth: 640, margin: '0 auto' }}>
            ...
            <div style={card}>
              ...
              <label style={{ ...eyebrow, display: 'block', marginTop: 18 }}>Електронна пошта</label>
              <input
                value={email}
              ...
              <div style={{ fontSize: 12, color: '#86868B', marginTop: 6 }}>
                Надішлемо підтвердження бронювання та повідомимо, якщо щось зміниться.
              </div>
            </div>
            ...
            {hasWeekendNight(checkIn, checkOut) && (
              <div
                style={{
                  marginTop: 18,
                  background: '#FDF6E7',
              ...
              >
                <div style={{ fontWeight: 600, marginBottom: 6 }}>Зверніть увагу</div>
                <p style={{ margin: '0 0 12px', fontSize: 14, lineHeight: 1.6, color: '#5C4D3F' }}>
                  Готель розташований на території комплексу з весільною залою. У пʼятницю та суботу тут часто
                  проходять урочистості, тому ввечері можливий підвищений рівень шуму. Просимо врахувати це,
                  плануючи відпочинок.
                </p>
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 14, cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={noiseAck}
                    onChange={(e) => setNoiseAck(e.target.checked)}
                    style={{ marginTop: 3 }}
                  />
                  <span>Я ознайомлений(а) і погоджуюсь</span>
                </label>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 22 }}>
              ...
              <button
                onClick={() => {
                  if (hasWeekendNight(checkIn, checkOut) && !noiseAck) {
                    show('Підтвердіть, що ознайомлені з попередженням про можливий шум.');
                    return;
                  }
                  doPay();
                }}
                disabled={paying}
                style={{ ...primaryBtnFull, opacity: paying ? 0.6 : 1 }}
              >
